...
namespace AmySurf.Tests
{
    public class ForecastProviderTest
    {
        ...
        public async Task TestLockOnRWHelper()
        {
            ...
            OfflineForecastProvider offlineForecastProvider = localHost.Services.GetRequiredService<OfflineForecastProvider>();
        ...
        private static IHostBuilder CreateHostBuilderTest(string[] args)
        {
            return Host.CreateDefaultBuilder(args)
                ...
                {
                    ServiceDescriptor sd = serviceCollection.FirstOrDefault(s => s.ImplementationFactory?.Method?.DeclaringType?.FullName?.Contains("AmySurf") == true);
                    serviceCollection.Remove(sd);
                    serviceCollection.AddHostedService<ForecastWorkerBackgroundService>(serviceProvider =>
                    {
                        return ActivatorUtilities.CreateInstance<ForecastWorkerBackgroundService>(serviceProvider, new TestForecastProvider());
                    });
